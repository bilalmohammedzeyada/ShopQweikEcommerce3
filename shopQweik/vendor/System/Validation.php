...
class Validation
{
    ...
    public function required($inputName, $customErrorMessage = null)
    {
        ...
        if ($inputValue === '') {
            $message = $customErrorMessage ?: sprintf('%s مطلوب', ucfirst($inputName));
    ...
    public function image($inputName, $customErrorMessage = null)
    {
        ...
        if (! $file->isImage()) {
            $message = $customErrorMessage ?: sprintf('%s ليست صورة صالحة', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function email($inputName, $customErrorMessage = null)
    {
        ...
        if (! filter_var($inputValue, FILTER_VALIDATE_EMAIL)) {
            $message = $customErrorMessage ?: sprintf('%s ليس بريد إلكتروني صالح', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    ...
    public function minLen($inputName, $length, $customErrorMessage = null)
    {
        ...
        if (strlen($inputValue) < $length) {
            $message = $customErrorMessage ?: sprintf('%s يجب أن يكون على الأقل %d', ucfirst($inputName), $length);
            $this->addError($inputName, $message);
        }
    ...
    public function maxLen($inputName, $customErrorMessage = null)
    {
        ...
        if (strlen($inputValue) > $length) {
            $message = $customErrorMessage ?: sprintf('%s يجب أن يكون على الأكثر %d', ucfirst($inputName), $length);
            $this->addError($inputName, $message);
        }
    ...
    public function match($firstInput, $secondInput, $customErrorMessage = null)
    {
        ...
        if ($firstInputValue != $secondInputValue) {
            $message = $customErrorMessage ?: sprintf('%s يجب أن تطابق %s', ucfirst($secondInput), ucfirst($firstInput));
    ...
    public function unique($inputName, array $databaseData, $customErrorMessage = null)
    {
        ...
        if ($result) {
            $message = $customErrorMessage ?: sprintf('%s موجود مسبقا', ucfirst($inputName));
            $this->addError($inputName, $message);
        }
    }
