...
abstract class Model
{
    ...
    {
        return $this->where('page != ?' , null)->orderBy('id', 'DESC')->fetchAll($this->table);
    }
	...
	public function deactive($value, $key = 'id')
    {
        return $this->data('status', 'disabled')->where($key .'=?' , $value)->update($this->table);
    }
	
	public function isactive($value, $key = 'id')
    {
        return $this->data('status', 'enabled')->where($key .'=?' , $value)->update($this->table);
    }
	
	public function deletesliders($value, $key = 'id')
    {
        return $this->data('status', 'enabled')->where($key .'=?' , $value)->update($this->table);
    }
